<template>
  <v-container fluid style="background-color: #e8e8e8e8">
    <v-row class="py-10 px-5">
      <v-col v-for="card in cards" :key="card.title" class="px-2">
        <v-card
          height="300"
          style="text-align: center"
          class="flex-column align-center justify-center py-10"
        >
          <v-card-image>
            <span style="text-align: center" v-html="card.svg"></span>
          </v-card-image>
          <v-card-text style="font-weight: bold" class="mb-0">
            {{ card.title }}
          </v-card-text>
          <v-card-text>
            {{ card.desc }}
